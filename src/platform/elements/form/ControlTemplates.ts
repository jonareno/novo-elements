  ...
  template: `
        ...
        <ng-template novoTemplate="textbox" let-control let-form="form" let-errors="errors" let-methods="methods">
          <div [formGroup]="form" class="novo-control-input-container novo-control-input-with-label" [tooltip]="control?.tooltip" [tooltipPosition]="control?.tooltipPosition"  [tooltipSize]="control?.tooltipSize" [tooltipPreline]="control?.tooltipPreline">
            ...
            <novo-number-textbox *ngIf="control?.type === 'number' && control?.subType !== 'percentage'" [value]="control.value" [name]="control?.key" [formControlName]="control?.key" [subType]="control?.subType" [config]="control?.config"></novo-number-textbox>
            <novo-number-textbox *ngIf="control?.type === 'number' && control?.subType === 'percentage'" [value]="control?.percentValue" [name]="control?.key" [formControlName]="control?.key" [subType]="control?.subType" [config]="control?.config" [placeholder]="control?.placeholder" [maxlength]="control?.maxlength" (focus)="methods.handleFocus($event)" (blur)="methods.handleBlur($event)"></novo-number-textbox>            
